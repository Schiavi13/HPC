...
int main(){
    ...
    pthread_t hilos[nucleos];  //Reserva de memoria para n*n hilos
...
void multiplicar_matrices(estructura_matrices * matrices, pthread_t * hilos, int ** matriz1, int ** matriz2, int ** matrizResultado, int n){
    ...
    for(int i=0;i<n;i++){
        for(int j=0;j<n;j++){
            ...
            */
            pthread_create(&(hilos[iterHilo]),NULL,producto_punto,(void *) matrices);
            if(iterHilo == (nucleos-1)){
                for(int k=0;k<nucleos;k++){
